...
class Trainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.train_metrics = MetricTracker(
            "train/loss", *["train/" + m.__name__ for m in self.metric_ftns]
        )
        self.valid_metrics = MetricTracker(
            "val/loss", *["val/" + m.__name__ for m in self.metric_ftns]
        )
    ...
    def train(self):
        ...
        for epoch, _ in enumerate(range(self.epochs), start=1):
            for _, data in enumerate(
                tqdm(self.data_loader, desc=f"TRAINING - [{epoch}] EPOCH")
            ):
                step += 1
                ...
                for met in self.metric_ftns:
                    self.train_metrics.update(
                        "train/" + met.__name__, met(preds, targets)
                    )
            ...
            for _, data in enumerate(self.valid_data_loader):
                ...
                for met in self.metric_ftns:
                    self.valid_metrics.update(
                        "val/" + met.__name__, met(preds, targets)
                    )
